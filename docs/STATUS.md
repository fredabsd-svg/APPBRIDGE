...
**Última atualização:** 2026-09-26 · **Sessão atual:** S020 · **Fase:** ✅ Design concluído → **implementação do MVP-0b**
...
> **23 ADRs** estão aceitos (13 no fechamento do design + ADR-0014 a ADR-0023; ADR-0021 a ADR-0023
> aceitos por decisão delegada de Frederico em 2026-09-26, S020). O replanejamento do cronograma foi aprovado na **opção A** e ratificado
...
> ADR-0022) fechou o ciclo de vida da sessão. Na S020, Frederico delegou as decisões pendentes: ADR-0021/0022
> aceitos e ADR-0023 (troca por access token recente e de uso único) implementado. Seis migrações
> aplicam no PostgreSQL local; os 67 testes passam com 88,78% de cobertura de linhas medida. Control Plane e Launcher compilam no SDK .NET 10.0.401. Isso conclui a fatia de software
...
| RC-02 | `IdentityTokenValidator` com escopo de requisição baixa os metadados do Entra a cada login | Média | **Corrigido na S020** — gerenciador em cache por autoridade no processo (ADR-0023 §5) |
| RC-03 | Duas instâncias do launcher podem renovar o mesmo refresh; o servidor vê replay e revoga a sessão (R-032) | Média | **Corrigido na S020** — trava de arquivo entre processos; a instância que chega depois adota o par já renovado |
...
| RC-05 | O ID token do Entra é aceito como credencial de troca; um token capturado compra uma sessão de 30 dias | Média | **Corrigido na S020 pelo ADR-0023** — resíduo em R-035 |
...
| [ADR-0023](adr/ADR-0023-troca-por-access-token-de-uso-unico.md) | Troca de sessão | **Substitui ADR-0017 §1.** Só access token do Entra para a API (`aud`, `scp`, `azp` do launcher), com no máximo 10 min de idade e uso único gravado em `identity_token_redemption` | API §2, SEGURANCA, MODELO-DE-DADOS |
| [ADR-0021](adr/ADR-0021-registro-de-sessao-no-lancamento.md) | Registro de sessão | **Aceito (delegado, S020).** `SessionRegistry` reutiliza a sessão do usuário, serializa por trava consultiva e grava sessão com vínculo pendente no lançamento concedido; `backend_session_id` passa a aceitar nulo | MODELO-DE-DADOS §6.2, ARQUITETURA §4.2 |
| [ADR-0022](adr/ADR-0022-reconciliacao-de-sessoes-com-o-connection-broker.md) | Reconciliação de sessões | **Aceito (delegado, S020).** `Get-RDUserSession` fora da transação, vínculo por SID, `reconciled_missing`/`never_connected` com backend disponível, só `stale_expired` sem ele; sessões não lançadas pelo AppBridge não são adotadas | ARQUITETURA §4.2, MODELO-DE-DADOS §6.2 |
...
| PRE-32 | 10 min de idade máxima do access token cobrem a demora entre o login interativo e a troca | ADR-0023 | medição no dogfood |
...
| R-035 | O ID token do Entra funcionava como credencial de portador na troca por sessão de até 30 dias (RC-05) | Baixa (era Média) | **Mitigado pelo ADR-0023** — resíduo: access token capturado antes do primeiro uso vale até 10 min; prova de posse fica para a V2 |
